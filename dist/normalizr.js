 ...
 */
function denormalizeImmutable(schema, input, updateCache, unvisit, getDenormalizedEntity) {
  return Object.keys(schema).reduce(function (object, key) {
    ...
    if (object.has(stringKey)) {
      var setObject = object.set(stringKey, unvisit(object.get(stringKey), schema[stringKey], getDenormalizedEntity));
      return updateCache(setObject);
    } else {
      return object;
    }
  }, input);
...
var EntitySchema = function () {
  ...
  createClass(EntitySchema, [{
    ...
    value: function denormalize(entityOrId, unvisit, getDenormalizedEntity) {
      ...
      return getDenormalizedEntity(this, entityOrId, function (entity, addToCache) {
        if ((typeof entity === 'undefined' ? 'undefined' : _typeof(entity)) !== 'object' || entity === null) {
          return entity;
        }

        if (isImmutable(entity)) {
          var entityId = entity.get(_this2._idAttribute);
          var preCachedEntity = addToCache(_this2.key, entityId, entity);
          var updateCache = function updateCache(entity) {
            return addToCache(_this2.key, entityId, entity);
          };
          var normalized = denormalizeImmutable(_this2.schema, preCachedEntity, updateCache, unvisit, getDenormalizedEntity);
          return addToCache(_this2.key, entityId, normalized);
        }

        var cachedEntity = addToCache(_this2.key, _this2.getId(entity), _extends({}, entity));
        Object.keys(_this2.schema).forEach(function (key) {
...
var normalize$1 = function normalize(schema, input, parent, key, visit, addEntity) {
...
var denormalize$1 = function denormalize(schema, input, unvisit, getDenormalizedEntity) {
  schema = validateSchema(schema);
...
var getEntities = function getEntities(entities, cache, isImmutable$$1) {
